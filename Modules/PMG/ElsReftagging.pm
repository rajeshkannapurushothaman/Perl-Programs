...
package ElsReftagging;

use strict;
use PMG::ElsAuthortag;
use Win32;
use Cwd;
use File::Basename;
...
sub year_conversion {
	...
	if($content !~ m/<Year>/) {
		if($content =~ m/ \(((18|19|20)[0-9][0-9][a-z]?)\)\.$/) {
		 ...
		 elsif($content =~ m/ ((:?18|19|20)[0-9]{2}[a-z]? $month)[\;\:\,]/) {
			$content =~ s/ ((?:18|19|20)[0-9]{2}[a-z]?) ($month)([\;\:\,])/ <Year>$1<\/Year> <month>$2<\/month>$3/;
		}
...
#print "page8,\n";
		...
		elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|\&\#x2013\;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)[\,\.]? PMID\: ?([A-Za-z]?[0-9]+[A-Za-z]?)/) {
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|\&\#x2013\;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)([\,\.]? PMID\: ?[A-Za-z]?[0-9]+[A-Za-z]?)/ <FirstPage>$1<\/FirstPage>&ndash;<LastPage>$3<\/LastPage>$4/;
		}
			...
			$content =~ s/\b([A-Za-z]?[0-9]+[A-Za-z]?)\:([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\.?)$/<Vol_No>$1<\/Vol_No>:<FirstPage>$2<\/FirstPage>&ndash;<LastPage>$4<\/LastPage>$5/;
		}
...
#print "pageLAst,\n";
	...
	$content =~ s/(PMID: ?)<FirstPage>([^><]+)<\/FirstPage>/$1$2/;
...
sub volIss_conversion {
	...
	if($content !~ m/<Vol_No>/ && $content !~ m/<Issue_No>/) {
      ...
      $content =~ s/\b([Vv]ol\.?) ([XVI]+) /$1 <Vol_No>$2<\/Vol_No> /i;
		} elsif($content =~ m/(\; ?| )([0-9]+\([0-9]+(?:\/[0-9]+)?\))(\, |\: ?)<FirstPage>/) {
			$content =~ s/(\; ?| )([0-9]+)\(([0-9]+(?:\/[0-9]+)?)\)((\, |\: ?)<FirstPage>)/$1<Vol_No>$2<\/Vol_No>\(<Issue_No>$3<\/Issue_No>\)$4/;
    }
...
sub arttitle_conversion {
	...
	if($content !~ m/<Article_Title>/) {
		...
		}elsif($content =~ m/\&\#x2018\;(.*?)\&\#x2019\;/) {
			#Punctuation removed
			$content =~ s/\&\#x2018\;(.*?)\&\#x2019\;/<Article_Title>$1<\/Article_Title>/;
		}
	}
...
sub publisherloc_tagging {
  my $content = shift;
  $content = &ElsReftagging::year_aftr_Publoc_conversion($content, "Year");
  $content = &ElsReftagging::year_aftr_Publoc_conversion($content, "Edition");
  $content = &ElsReftagging::year_aftr_Publoc_conversion($content, "FirstPage");
  return $content;
}

sub publishername_tagging {
  my $content = shift;
  $content = &ElsReftagging::Publoc_aftr_Pubname_conversion($content);
  return $content;
}
...
sub journal_conversion {
  ...
  $journal = ElsAuthortag::AuGroupIdentification($journal);
...
sub book_conversion {
  ...
  $book = &ElsAuthortag::EdGroupIdentification($book);
  $book = ElsAuthortag::AuGroupIdentification($book);
...
#  }
  ...
  $book = ElsReftagging::partial_tagging($book);
  $book = &ElsReftagging::restrict_book_conversion($book);
...
sub conference_conversion {
  ...
  $conference = &ElsAuthortag::EdGroupIdentification($conference);
  $conference = ElsAuthortag::AuGroupIdentification($conference);
...
#    }
  ...
  $conference = ElsReftagging::partial_tagging($conference);
  #$conference = &ElsReftagging::restrict_book_conversion($conference);
...
sub thesis_conversion {
  ...
  $thesis = ElsAuthortag::AuGroupIdentification($thesis);
  # SPIE style
...
#    }
  ...
  $thesis = ElsReftagging::partial_tagging($thesis);
  return $thesis;
}
...
sub report_conversion {
  ...
  $report = ElsAuthortag::AuGroupIdentification($report);
...
# Partial country identification
  ...
  $report = ElsReftagging::partial_tagging($report);
  ...
  my $references = shift;
    ...
    if ($references =~ m/ \(([0-9]{4}[a-z]?)\)/) {
      ...
      $references =~ s/ ((?:19|20)[0-9]{2}[a-z]?)([\.\,]? &ldquo;)/ <Year>$1<\/Year>$2/;
   	...
   	$references = &ElsAuthortag::EdGroupIdentification($references);
    $references = ElsAuthortag::AuGroupIdentification($references);
...
#    }
    ...
    $references = &ElsReftagging::partial_tagging($references);
    	#print $references,"\n\n";

    $references = &ElsReftagging::restrict_book_conversion($references);
    $references = &ElsReftagging::publisherloc_tagging($references);

    $references = &ElsReftagging::publishername_tagging($references);
  return $references;
}
my $count = 0;
sub Cleanup {
