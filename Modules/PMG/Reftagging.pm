...
sub partial_tagging {
	...
	$content = fpage_conversion($content);
...
sub final_cleanup {
	...
	$content =~ s/(\.)<\/lpage>/<\/lpage>$1/gi;
	$content =~ s/ ([A-Za-z]+\.(com|edu|org))\b/ <Website>$1<\/Website>/gi;
	$content =~ s/([A-Za-z]+\, (?:$month)\,? )<fpage>([0-9]{1,2})<\/fpage>\.$/$1$2\./i;
	$content =~ s/([A-Za-z]+\, (?:$month)\,? )<fpage>([0-9]{1,2})<\/fpage>&ndash;<lpage>([0-9]{1,2})<\/lpage>\.$/$1$2\&ndash\;$3\./i;
	$content =~ s/<article-title>(\)\,?\s*)/$1<article-title>/i;
	$content =~ s/([Vv]ol\.? <volume>(?:[0-9]+)<\/volume>\,? [Nn]o\.? )<fpage>([A-Z0-9]+)<\/fpage>/$1<issue>$2<\/issue>/;
	$content =~ s/(\,? [Vv]ol\.?)(<\/Journal_Title>)/$2$1/;
	$content =~ s/<\/?error>//gi;
	$content =~ s/\&comma(<\/[a-z]+>)\;/$1\,/gi;
	$content =~ s/\&([a-z]+)(<\/[a-z]+>)\;/\&$1\;$2/gi;
	$content =~ s/\b(vol\. )([A-Z]?[0-9]+(?:\-?[A-Z])?)(\, no\. )<volume>([0-9]+)<\/volume>/$1<volume>$2<\/volume>$3<issue>$4<\/issue>/;
	
	$content =~ s/(\. [Vv]ol\.? )([A-Z]?[0-9]+(?:\-?[A-Z])?) /$1<volume>$2<\/volume> /;
	$content =~ s/<Journal_Title>In<\/Journal_Title> (<em>.*?<\/em>)/In <Journal_Title>$1<\/Journal_Title>/s;
	
	$content =~ s/ ($month) <fpage>([0-9]{1,2})<\/fpage>/ $1 $2/i;
	
	if($content =~ m/<article-title>(.*?)<\/article-title>/i) {
	  my $Art_Tit = $&;
	  $Art_Tit =~ s/<Journal_Title>(.*?)<\/Journal_Title>/<i>$1<\/i>/isg;
	  if ($Art_Tit =~ s/<Journal_Title>//i){
	    my $jrn_Tit = $&;
      $content =~s/<article-title>(.*?)<\/article-title>/$Art_Tit.$jrn_Tit/isg;
	  }elsif($Art_Tit =~ s/<\/Journal_Title>//i){
	    my $jrn_Title = $&;
	    $content =~s/<article-title>(.*?)<\/article-title>/$jrn_Title.$Art_Tit/isg;
	  }else{
	    $content =~s/<article-title>(.*?)<\/article-title>/$Art_Tit/isg;
	  }
	}
	...
	$content =~ s/\)<\/Doi>/<\/Doi>\)/g;
	$content =~ s/<Doi>\(/\(<Doi>/g;
	$content =~s/\:<\/article-title>/<\/article-title>\:/g;
	#print $content,"<<<<<<<<<<<\n";
	return $content;
}
...
sub final_vol_conversion {
	my $content = shift;
		#print $content,"\n\n";
		$content =~ s/([\.\,\>] )([0-9]+)(\, pp?\. )<fpage>/$1<volume>$2<\/volume>$3<fpage>/ if($content !~ m/<volume>/);

		$content =~ s/([\.\,\>] )([0-9]+\/[0-9]+)( ?\: )<fpage>/$1<volume>$2<\/volume>$3<fpage>/ if($content !~ m/<volume>/);
		
		$content =~ s/([\.\,\>] )([0-9]+)\.([0-9]+)\, <fpage>/$1<volume>$2<\/volume>\.<issue>$3<\/issue>\, <fpage>/ if($content !~ m/<volume>/);
		
		$content =~ s/([\.\,\>] )([0-9]+)\.([0-9]+)(\,? \((?:$month) <Year>(?:18|19|20)[0-9][0-9][a-z]?<\/Year>\)[\.\,] <fpage>)/$1<volume>$2<\/volume>\.<issue>$3<\/issue>$4/ if($content !~ m/<volume>/);
		
		$content =~ s/([\.\,\>] )([0-9]+)\.([0-9]+)(\,? \(<Year>(?:18|19|20)[0-9][0-9][a-z]?<\/Year>\)[\.\,] <fpage>)/$1<volume>$2<\/volume>\.<issue>$3<\/issue>$4/ if($content !~ m/<volume>/);
	
		# Volume based Journal title conversion
		#print $content,"\n";
		#$content =~ s/<Authorgroup>(.*?)<\/Authorgroup>([\,\.] <volume>)/<Journal_Title>$1<\/Journal_Title>$2/;
		return $content;
}
...
sub fpage_conversion {
	my $content = shift;
	#print $content,"\n";
		# First page and lpage checking and tagging...
...
#print "page1,\n";
			$content =~ s/( [Pp]p\. ?)([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,?)/$1<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
...
#print "page3,\n";
			$content =~ s/( [Pp]\. ?)([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,?)\b/$1 <fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
		} elsif($content =~ m/ [Pp]p\.? ((?:[A-Z])?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\,?\b/) {
#print "page4,\n";
			$content =~ s/( [Pp]p\.? )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,?)\b/$1 <fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
		} elsif($content =~ m/ [Pp][\.p]+ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\,? \(?<Year>/) {
#print "page5,\n";
			$content =~ s/( [Pp][\.p]+ )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,? \(?<Year>)/$1<fpage>$2<\/fpage>$3/;
		} elsif($content =~ m/ [Pp][\.p]+ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,|\.$)/) {
#print "page6,\n";
			$content =~ s/( [Pp][\.p]+ )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,|\.$)/$1<fpage>$2<\/fpage>$3/;
		} elsif($content =~ m/ [Pp]p\.?([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,|\.$)/) {
#print "page7,\n";
			$content =~ s/( [Pp]p\.?)([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,|\.$)/$1 <fpage>$2<\/fpage>$3/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\,? ($month(?:\-$month)?) \(?<Year>/) {
#print "page8,\n";
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,? ($month(?:\-$month)?) \(?<Year>)/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)[\,\.] [A-z]+[\,\.]? \(?<Year>/) {
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)([\,\.] [A-z]+[\,\.]? \(?<Year>)/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+\-[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+\-[0-9]+[A-Za-z]?|XXX)\,? \(?<Year>/) {
			$content =~ s/ ([A-Za-z]?[0-9]+\-[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+\-[0-9]+[A-Za-z]?|XXX)(\,? \(?<Year>)/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
		} 
		elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|\&\#x2013\;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\,? \(?<Year>/) {
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|\&\#x2013\;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,? \(?<Year>)/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
		}
		 elsif($content =~ m/ ([xivdcXIVDC]+)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([xivdcXIVDC]+)\.$/) {
			$content =~ s/ ([xivdcXIVDC]+)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([xivdcXIVDC]+)\.$/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>\./;
		}
		
		 elsif($content =~ m/<strong><em>[A-Z]?[0-9]+[A-Z]?<\/em><\/strong>\(([A-Z]?[0-9\-]+[A-Z]?)\)[\,\:]?\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,? \(?<Year>)/) {
			$content =~ s/(<strong><em>[A-Z]?[0-9]+[A-Z]?<\/em><\/strong>\([A-Z]?[0-9\-]+[A-Z]?\)[\,\:]?\s*)([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,? \(?<Year>)/$1<fpage>$2<\/fpage>$3/;
		}
		
		 elsif($content =~ m/<strong><em>[A-Z]?[0-9]+[A-Z]?<\/em><\/strong>[\,\:]?\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,? \(?<Year>)/) {
			$content =~ s/(<strong><em>[A-Z]?[0-9]+[A-Z]?<\/em><\/strong>[\,\:]?\s*)([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,? \(?<Year>)/$1<fpage>$2<\/fpage>$3/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)( ?\-\-? ?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)( [pP]{1,2}\.$)/) {
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)( ?\-\-? ?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)( [pP]{1,2}\.$)/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?)( [pP]{1,2}\.$)/) {
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?)( [pP]{1,2}\.$)/ <fpage>$1<\/fpage>$2/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)( [pP]{1,2}[\.\;]+ )/) {
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)( [pP]{1,2}[\.\;]+ )/ <fpage>$1<\/fpage>$2/;
		} elsif($content =~ m/ ((?:[A-Z])?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\.(PMGQ[0-9]+)?$/) {
			$content =~ s/( )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\.(PMGQ[0-9]+)?)$/$1<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)[\.\;]$/) {
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)([\.\;])$/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)[\.\;] /) {
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)([\.\;]) / <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4 /;
		}
		
		elsif($content =~ m/\b([A-Za-z]?[0-9]+[A-Za-z]?)\:([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\.$/) {
			$content =~ s/\b([A-Za-z]?[0-9]+[A-Za-z]?)\:([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\.$/<volume>$1<\/volume>:<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>\./;
		}
		
		elsif($content =~ m/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\./) {
			$content =~ s/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\./$1<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>\./;
		}
		
		elsif($content =~ m/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX) ?\(<Year>/) {
			$content =~ s/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)( ?\(<Year>)/$1<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
		}
		
		elsif($content =~ m/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\. (?:doi|DOI)\:)/) {
			$content =~ s/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\. (?:doi|DOI)\:)/$1<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
		} elsif($content =~ m/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\.$)/) {
			$content =~ s/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\.$)/$1<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
		}
		
		elsif($content =~ m/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\. (?:doi|DOI)\:)/) {
			$content =~ s/(\: ?| )([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\. (?:doi|DOI)\:)/$1<fpage>$2<\/fpage>$3/;
		}
		
		
		elsif($content =~ m/ \(pp.? ((?:[A-Z])?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\)/) {
			$content =~ s/( \(pp.? )((?:[A-Z])?[0-9]+[A-Za-z]?|XXX)\s*(\-\-?|&ndash;|&minus;|&mdash;|&\#x2013;)\s*([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\)/$1<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>\)/;
		} elsif($content =~ m/ \([0-9]+ [pP]p\.?\)/) {
			$content =~ s/ \(([0-9]+)( [pP]p\.?)\)/ \(<fpage>$1<\/fpage>$2\)/;
		} elsif($content =~ m/[\,\:] ([0-9]+)\.$/) {
			$content =~ s/([\,\:] )([0-9]+)\.$/$1<fpage>$2<\/fpage>\./;
		} elsif($content =~ m/\b([0-9]+)\:([0-9]+)\.$/) {
			$content =~ s/\b([0-9]+)\:([0-9]+)\.$/<volume>$1<\/volume>:<fpage>$2<\/fpage>\./;
		} elsif($content =~ m/[\,\:] ?([0-9]+)$/) {
			$content =~ s/([\,\:] ?)([0-9]+)$/$1<fpage>$2<\/fpage>/;
		} elsif($content =~ m/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)\,? \(?<Year>/) {
#print "pageLAst,\n";
			$content =~ s/ ([A-Za-z]?[0-9]+[A-Za-z]?|XXX)(\,? \(?<Year>)/ <fpage>$1<\/fpage>$2/;
		} elsif($content =~ m/[\,\;\:]([a-z][0-9]+)\.$/) {
			$content =~ s/([\,\;\:])([a-z][0-9]+)\.$/$1<fpage>$2<\/fpage>\./;
		} elsif($content =~ m/[\,\;\:]([0-9]+)\.$/) {
			$content =~ s/([\,\;\:])([0-9]+)\.$/$1<fpage>$2<\/fpage>\./;
		}
		
		 $content =~ s/<fpage>((?:18|19|20)[0-9][0-9])<\/fpage>( pp?. )([0-9]+)/$1$2<fpage>$3<\/fpage>/;
	}
	return $content;
}
...
sub volIss_conversion {
	...
	if($content !~ m/<volume>/ && $content !~ m/<issue>/) {
		if ($content =~ m/[\.\,] [Vv]ol\. ?([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)[\,\.] (?:[Nn]o|Is)\.? ?([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;|XXXampXXX)[0-9]+)?)\b/) {
      $content =~ s/([\.\,] [Vv]ol\. ?)([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)(\,|\.)( (?:[Nn]o|Is)\.? ?)([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;|XXXampXXX)[0-9]+)?)\b/$1<volume>$2<\/volume>$3$4<issue>$5<\/issue>/;
    } elsif ($content =~ m/[\.\,] [Vv]ol\. ?([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)[\,\.] (?:[Nn]o|Is)\. ?([0-9]+)\b/) {
      $content =~ s/([\.\,] [Vv]ol\. ?)([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)(\,|\.)( (?:[Nn]o|Is)\. ?)([0-9]+)\b/$1<volume>$2<\/volume>$3$4<issue>$5<\/issue>/;
    } elsif ($content =~ m/[\.\,] [Vv]\. ?([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)[\,\.] (?:[Nn]o|Is)\. ?([0-9]+)\b/) {
      $content =~ s/([\.\,] [Vv]\. ?)([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)(\,|\.)( (?:[Nn]o|Is)\. ?)([0-9]+)\b/$1<volume>$2<\/volume>$3$4<issue>$5<\/issue>/;
    } elsif ($content =~ m/[\.\,] [Vv]olume ?([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)[\,\.] (?:[Nn]o|Is)\. ?([0-9]+)\b/) {
      $content =~ s/([\.\,] [Vv]olume ?)([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)(\,|\.)( (?:[Nn]o|Is)\. ?)([0-9]+)\b/$1<volume>$2<\/volume>$3$4<issue>$5<\/issue>/;
    } elsif ($content =~ m/[\.\,] [Vv]olume ([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)[\,\. ]/) {
      $content =~ s/([\.\,] [Vv]olume )([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)(\,|\.| )/$1<volume>$2<\/volume>$3/;
    } elsif ($content =~ m/ ([Vv]ol\.?) ([0-9]+)\, (No\.?|Is\.?) ([0-9]+)[\,\;]/i) {
      $content =~ s/ ([Vv]ol\.?) ([0-9]+)\, (No\.?|Is\.?) ([0-9]+)([\,\;])/ $1 <volume>$2<\/volume>\, $3 <issue>$4<\/issue>$5/i;
		} elsif ($content =~ m/ ([Vv]ol\.?) ([0-9]+) (No\.?|Is\.?) ([0-9]+)[\,\;]/i) {
      $content =~ s/ ([Vv]ol\.?) ([0-9]+) (No\.?|Is\.?) ([0-9]+)([\,\;])/ $1 <volume>$2<\/volume> $3 <issue>$4<\/issue>$5/i;
		} elsif ($content =~ m/ ([Vv]ol\.?) ([A-Z]+\-?[0-9]+)\, ([Nn]o\.?) ([0-9]+)[\,\;]/i) {
			$content =~ s/ ([Vv]ol\.?) ([A-Z]+\-?[0-9]+)\, ([Nn]o\.?) ([0-9]+)([\,\;])/ $1 <volume>$2<\/volume>\, $3 <issue>$4<\/issue>$5/i;
		} elsif ($content =~ m/ ([Vv]ol\.?) ([A-Z]+\-?[0-9]+) ([Nn]o\.?) ([0-9]+)[\,\;]/i) {
			$content =~ s/ ([Vv]ol\.?) ([A-Z]+\-?[0-9]+) ([Nn]o\.?) ([0-9]+)([\,\;])/ $1 <volume>$2<\/volume> $3 <issue>$4<\/issue>$5/i;
		} elsif ($content =~ m/ ([Vv]ol\.?) ([XVI]+)\, (No\.?|Is\.?) ([0-9]+)[\,\;]/i) {
      $content =~ s/ ([Vv]ol\.?) ([XVI]+)\, (No\.?|Is\.?) ([0-9]+)([\,\;])/ $1 <volume>$2<\/volume>\, $3 <issue>$4<\/issue>$5/i;
		} elsif ($content =~ m/ ([Vv]ol\.?) ([XVI]+) (No\.?|Is\.?) ([0-9]+)[\,\;]/i) {
      $content =~ s/ ([Vv]ol\.?) ([XVI]+) (No\.?|Is\.?) ([0-9]+)([\,\;])/ $1 <volume>$2<\/volume> $3 <issue>$4<\/issue>$5/i;
		} elsif ($content =~ m/[\.\,] [Vv]\. ?([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?) ?\(([0-9]+)\)\,/) {
      $content =~ s/([\.\,] [Vv]\. ?)([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?) ?\(([0-9]+)\)\,/$1<volume>$2<\/volume> \(<issue>$3<\/issue>\)/;
    } elsif ($content =~ m/[\.\,] [Vv]ol\. ([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)[\,\.]/) {
      $content =~ s/([\.\,] [Vv]ol\. )([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)(\,|\.)/$1<volume>$2<\/volume>$3/;
    } elsif ($content =~ m/[\.\,] [Vv]\. ([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)[\,\.\:]/) {
      $content =~ s/([\.\,] [Vv]\. )([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)(\,|\.|\:)/$1<volume>$2<\/volume>$3/;
    } elsif($content =~ m/<strong><em>[A-Z]?[0-9]+[A-Z]?<\/em><\/strong>(\s*\()([A-Z]?[0-9\-]+[A-Z]?)\)[\,\:]?\s*<fpage>/) {
			$content =~ s/<strong><em>([A-Z]?[0-9]+[A-Z]?)<\/em><\/strong>(\s*\()([A-Z]?[0-9\-]+[A-Z]?)(\)[\,\:]?\s*<fpage>)/<volume>$1<\/volume>$2<issue>$3<\/issue>$4/;
		}	elsif($content =~ m/<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>(\,\s*no\. )([0-9\-]+)\,\s*<fpage>/) {
			$content =~ s/<strong>([A-Z]?[0-9]+[A-Z]?)<\/strong>(\,\s*no\. )([0-9\-]+)(\,\s*<fpage>)/<volume><strong>$1<\/strong><\/volume>$2<issue>$3<\/issue>$4/;
		}	elsif($content =~ m/ [A-Z]?[0-9]+[A-Z]?(\,\s*no\. )([0-9\-]+)\,\s*<fpage>/) {
			$content =~ s/ ([A-Z]?[0-9]+[A-Z]?)(\,\s*no\. )([0-9\-]+)(\,\s*<fpage>)/<volume>$1<\/volume>$2<issue>$3<\/issue>$4/;
		} elsif($content =~ m/<strong><em>[A-Z]?[0-9]+[A-Z]?<\/em><\/strong>(\,? |\:)<fpage>/) {
			$content =~ s/<strong><em>([A-Z]?[0-9]+[A-Z]?)<\/em><\/strong>((\,? |\:)<fpage>)/<volume>$1<\/volume>$2/;
		} elsif($content =~ m/<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>(\s*\()([A-Z]?[0-9\-]+[A-Z]?)\)[\,\:]?\s*<fpage>/) {
			$content =~ s/(<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>)(\s*\()([A-Z]?[0-9\-]+[A-Z]?)(\)[\,\:]?\s*<fpage>)/<volume>$1<\/volume>$2<issue>$3<\/issue>$4/;
		} elsif($content =~ m/ [A-Z]?[0-9]+[A-Z]?(\s*\()([A-Z]?[0-9\-]+[A-Z]?)\)[\,\:]?\s*<fpage>/) {
			$content =~ s/ ([A-Z]?[0-9]+[A-Z]?)(\s*\()([A-Z]?[0-9\-]+[A-Z]?)(\)[\,\:]?\s*<fpage>)/ <volume>$1<\/volume>$2<issue>$3<\/issue>$4/;
		} elsif($content =~ m/ [A-Z]?[0-9]+[A-Z]?(\s*\()([A-Z]?[0-9\-]+[A-Z]?)\)[\,\:]?\s*/) {
			$content =~ s/ ([A-Z]?[0-9]+[A-Z]?)(\s*\()([A-Z]?[0-9\-]+[A-Z]?)(\)[\,\:]?\s*)/ <volume>$1<\/volume>$2<issue>$3<\/issue>$4/;
		} elsif($content =~ m/<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>\(([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)\)\: [Pp]p?\./) {
			$content =~ s/<strong>([A-Z]?[0-9]+[A-Z]?)<\/strong>\(([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)\)(\: [Pp]p?\.)/<volume>$1<\/volume>\(<issue>$2<\/issue>\)$3/;
		} elsif($content =~ m/<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>(\s*\()([A-Z]?[0-9\-]+[A-Z]?)\)[\,\:]?\s*(?:$month)(\s*\(?<Year>)/) {
			$content =~ s/(<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>)(\s*\()([A-Z]?[0-9\-]+[A-Z]?)(\)[\,\:]?\s*)((?:$month)\s*\(?<Year>)/<volume>$1<\/volume>$2<issue>$3<\/issue>$4$5/;
		} elsif($content =~ m/<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>\,? [0-9]+ \(?<fpage>/) {
			$content =~ s/<strong>([A-Z]?[0-9]+[A-Z]?)<\/strong>(\,? [0-9]+ \(?<fpage>)/<volume>$1<\/volume>$2/;
		} elsif($content =~ m/<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>[\,\:]? <fpage>/) {
			$content =~ s/<strong>([A-Z]?[0-9]+[A-Z]?)<\/strong>([\,\:]? <fpage>)/<volume>$1<\/volume>$2/;
		}
		
		 elsif($content =~ m/ [A-Z]?[0-9]+[A-Z]? \(<Year>([^><]+)<\/Year>\) <fpage>/) {
			$content =~ s/ ([A-Z]?[0-9]+[A-Z]?)( \(<Year>([^><]+)<\/Year>\) <fpage>)/ <volume>$1<\/volume>$2/;
		}
		
		
		elsif($content =~ m/<strong>[A-Z]?[0-9]+[A-Z]?<\/strong>[\,\:]? \(?<Year>/) {
			$content =~ s/<strong>([A-Z]?[0-9]+[A-Z]?)<\/strong>([\,\:]? \(?<Year>)/<volume>$1<\/volume>$2/;
		}	elsif($content =~ m/( \()([0-9]+)( ?\: ?)([0-9]+)(\)\, <Year>[0-9]+<\/Year>\, (?:pp|<fpage>))/) {
			$content =~ s/( \()([0-9]+)( ?\: ?)([0-9]+)(\)\, <Year>[0-9]+<\/Year>\, (?:pp|<fpage>))/$1<volume>$2<\/volume>$3<issue>$4<\/issue>$5/;
    } elsif($content =~ m/ ([XIVCDLMHUET]+)(\,? \(?(?:$month) and (?:$month)\, <Year>[0-9]+<\/Year>)/) {
			$content =~ s/ ([XIVCDLMHUET]+)(\,? \(?(?:$month) and (?:$month)\, <Year>[0-9]+<\/Year>)/ <volume>$1<\/volume>$2/;
    } elsif($content =~ m/ ([XIVCDLMHUET]+)(\,? \(?(?:$month) [0-9]{1,2}\, <Year>[0-9]+<\/Year>)/) {
			$content =~ s/ ([XIVCDLMHUET]+)(\,? \(?(?:$month) [0-9]{1,2}\, <Year>[0-9]+<\/Year>)/ <volume>$1<\/volume>$2/;
    } elsif($content =~ m/ ([XIVCDLMHUET]+)(\,? \(?(?:$month)\,? <Year>[0-9]+<\/Year>)/) {
			$content =~ s/ ([XIVCDLMHUET]+)(\,? \(?(?:$month)\,? <Year>[0-9]+<\/Year>)/ <volume>$1<\/volume>$2/;
    } elsif($content =~ m/ ([XIVCDLMHUET]+)(\,? \(?<Year>[0-9]+<\/Year>)/) {
			$content =~ s/ ([XIVCDLMHUET]+)(\,? \(?<Year>[0-9]+<\/Year>)/ <volume>$1<\/volume>$2/;
    }	elsif($content =~ m/ ([A-Z]{2,3})(\,? \(?(?:$month) [0-9]{1,2}\, <Year>[0-9]+<\/Year>)/) {
			$content =~ s/ ([A-Z]{2,3})(\,? \(?(?:$month) [0-9]{1,2}\, <Year>[0-9]+<\/Year>)/ <volume>$1<\/volume>$2/;
    } elsif($content =~ m/ ([A-Z]{2,3})(\,? \(?(?:$month)\,? <Year>[0-9]+<\/Year>)/) {
			$content =~ s/ ([A-Z]{2,3})(\,? \(?(?:$month)\,? <Year>[0-9]+<\/Year>)/ <volume>$1<\/volume>$2/;
    } elsif($content =~ m/ ([A-Z]{2,3})(\,? \(?<Year>[0-9]+<\/Year>)/) {
			$content =~ s/ ([A-Z]{2,3})(\,? \(?<Year>[0-9]+<\/Year>)/ <volume>$1<\/volume>$2/;
    }	elsif($content =~ m/( \()([0-9]+)( ?\: ?)([0-9]+)(\)\, <Year>[0-9]+<\/Year>\.?)$/) {
			$content =~ s/( \()([0-9]+)( ?\: ?)([0-9]+)(\)\, <Year>[0-9]+<\/Year>\.?)$/$1<volume>$2<\/volume>$3<issue>$4<\/issue>$5/;
    } elsif($content =~ m/ ([0-9]+)\:([0-9]+)(\, <fpage>)/) {
			$content =~ s/ ([0-9]+)\:([0-9]+)(\, <fpage>)/ <volume>$1<\/volume>\:<issue>$2<\/issue>$3/;
    } elsif ($content =~ m/\b([Vv]ol\.?) ([XVI]+)[\,\;\.]/i) {
      $content =~ s/\b([Vv]ol\.?) ([XVI]+)([\,\;\.])/$1 <volume>$2<\/volume>$3/i;
		} elsif ($content =~ m/\b([Vv]ol\.?) ([XVI]+) /i) {
      $content =~ s/\b([Vv]ol\.?) ([XVI]+) /$1 <volume>$2<\/volume> /i;
		} elsif($content =~ m/(\; ?| )([0-9]+\([0-9]+(?:\/[0-9]+|(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?\))(\, |\: ?)<fpage>/) {
			$content =~ s/(\; ?| )([0-9]+)\(([0-9]+(?:\/[0-9]+|(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)\)((\, |\: ?)<fpage>)/$1<volume>$2<\/volume>\(<issue>$3<\/issue>\)$4/;
    }
     elsif($content =~ m/(\; ?| )([0-9]+)(\, |\: ?)<fpage>/) {
			$content =~ s/(\; ?| )([0-9]+)((\, |\: ?)<fpage>)/$1<volume>$2<\/volume>$3/;
    }
    
    elsif($content =~ m/(\; ?| )([0-9]+)( ?\()(Suppl\.? [0-9]+)\)( ?\: ?)(<fpage>)/) {
			$content =~ s/(\; ?| )([0-9]+)( ?\()(Suppl\.? [0-9]+)(\) ?\: ?<fpage>)/$1<volume>$2<\/volume>$3<issue>$4<\/issue>$5/;
    }
    	elsif($content =~ m/(\; ?| )([0-9]+) (Suppl)( ?\: ?)(<fpage>)/) {
			$content =~ s/(\; ?| )([0-9]+) (Suppl)( ?\: ?<fpage>)/$1<volume>$2<\/volume> <issue>$3<\/issue>$4/;
    }	elsif($content =~ m/(\; ?| )[0-9]+\([0-9]+ Suppl\) ?\: ?<fpage>/) {
			$content =~ s/(\; ?| )([0-9]+)\(([0-9]+ Suppl)\)( ?\: ?<fpage>)/$1<volume>$2<\/volume>\(<issue>$3<\/issue>\)$4/;
    }
    
    
     elsif($content =~ m/ ([0-9]{1,3})\, ([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)\, <fpage>/) {
			$content =~ s/ ([0-9]{1,3})\, ([0-9]+(?:(?:\-\-?|&ndash;|&minus;|&mdash;)[0-9]+)?)(\, <fpage>)/ <volume>$1<\/volume>\, <issue>$2<\/issue>$3/;
    } elsif($content =~ m/ ([0-9]+)(\, |\:)<fpage>/) {
			$content =~ s/ ([0-9]+)((\, |\:)<fpage>)/ <volume>$1<\/volume>$2/;
    }	elsif($content =~ m/ [A-Z]?[0-9]+[A-Z]?[\,\:] <fpage>/) {
			$content =~ s/ ([A-Z]?[0-9]+[A-Z]?)([\,\:] <fpage>)/ <volume>$1<\/volume>$2/;
		} elsif($content =~ m/ ([0-9]+) [vV]ols\./) {
			$content =~ s/ ([0-9]+)( [vV]ols\.)/ <volume>$1<\/volume>$2/;
    }

	}
	#print $content,"\n";
	if($content !~ m/<issue>/) {
		if($content =~ m/<\/volume>, (?:[Nn]o|Is)\. [0-9]+/i) {
			$content =~ s/(<\/volume>, (?:[Nn]o|Is)\. )([0-9]+)/$1<issue>$2<\/issue>/i;
		}
		$content =~ s/([0-9]+) (Suppl\.? )<volume>([0-9]+)<\/volume>/<volume>$1<\/volume> <issue>$2$3<\/issue>/;
	}
	
	$content =~ s/\b([Vv]ol\.)(<volume>)/$1 $2/;
	#print $content,"\n";
	return $content;
}
...
sub vol_after_iss {
	...
	if($content !~ m/<issue>/) {
		 if ($content =~ m/<\/volume>\, ([0-9]+)([\,\;\.])/i) {
      $content =~ s/<\/volume>\, ([0-9]+)([\,\;\.])/<\/volume>\, <issue>$1<\/issue>$2/i;
		}
	}
	return $content;
}
...
sub year_after_vol {
	my $content = shift;
		# issue checking and tagging...
	if($content !~ m/<Year>/) {
		if ($content =~ m/((18|19|20)[0-9][0-9][a-z]?) ($month)( [0-9]+)\;<volume>/i) {
      $content =~ s/((?:18|19|20)[0-9][0-9][a-z]?) ($month)( [0-9]+)\;<volume>/<Year>$1<\/Year> <month>$2<month>$3\;<volume>/i;
		} elsif ($content =~ m/((18|19|20)[0-9][0-9][a-z]?) ($month)\;<volume>/i) {
      $content =~ s/((?:18|19|20)[0-9][0-9][a-z]?) ($month)\;<volume>/<Year>$1<\/Year> <month>$2<month>\;<volume>/i;
		}
	}
	return $content;
}
...
sub journaltitle_conversion {
	...
	if($content !~ m/<Journal_Title>/) {
		...
		} elsif($content =~ m/<\/article-title>\,?&rdquo; Proc\. [^\(\)\,]+[\(\)\,]/) {
			$content =~ s/(<\/article-title>\,?&rdquo; )(Proc\. [^\(\)\,]+)([\(\)\,])/$1<Journal_Title>$2<\/Journal_Title>$3/;
		} elsif($content =~ m/<\/article-title>\,?&rdquo; [A-z \.]+[\(\)\,]/) {
			$content =~ s/(<\/article-title>\,?&rdquo; )([A-z \.]+)([\(\)\,])/$1<Journal_Title>$2<\/Journal_Title>$3/;
		} elsif($content =~ m/<\/article-title>\,?&quot; [A-z \.]+[\(\)\,]/) {
			$content =~ s/(<\/article-title>\,?&quot; )([A-z \.]+)([\(\)\,])/$1<Journal_Title>$2<\/Journal_Title>$3/;
		} elsif($content =~ m/<em>(.*?)<\/em>[\,\.]?\s*/) {
		  my $space = $1;
			$content =~ s/<em>(.*?)<\/em>([\,\.]?\s*)/<Journal_Title>$1<\/Journal_Title>$2/ if ($content !~ m/<ConferenceName>/ && $space =~m/ /);
		}
#		elsif($content =~ m/\: [A-Z][^\:\,\>\<]+[\(\)\,]? <volume>/) {
#			$content =~ s/\: ([A-Z][^\:\,\>\<]+)([\(\)\,]? <volume>)/\: <Journal_Title>$1<\/Journal_Title>$2/;
#		}
#		 elsif($content !~ m/<volume>/ && $content =~ m/\: [^\:\,\>\<]+[\(\)\,]? <fpage>/) {
#			$content =~ s/\: ([^\:\,\>\<]+)([\(\)\,]? <fpage>)/\: <Journal_Title>$1<\/Journal_Title>$2/;
#		}
	}
...
sub arttitle_conversion {
	...
	if($content !~ m/<article-title>/) {
		if($content =~ m/\&ldquo\;(.*?)\&rdquo\;/) {
			$content =~ s/\&ldquo\;(.*?)\&rdquo\;/\&ldquo\;<article-title>$1<\/article-title>\&rdquo\;/;
		}elsif($content =~ m/\&\#34\;(.*?)\&\#34\;/) {
			$content =~ s/\&\#34\;(.*?)\&\#34\;/\&\#34\;<article-title>$1<\/article-title>\&\#34\;/;
		}elsif($content =~ m/\&quot\;(.*?)\&quot\;/) {
			$content =~ s/\&quot\;(.*?)\&quot\;/\&quot\;<article-title>$1<\/article-title>\&quot\;/;
		}elsif($content =~ m/\&lsquo\;(.*?)\&rsquo\;/) {
			$content =~ s/\&lsquo\;(.*?)\&rsquo\;/\&lsquo\;<article-title>$1<\/article-title>\&rsquo\;/;
		}elsif($content =~ m/\&lsquo\;(.*?)XXXrsquoXXX/) {
			$content =~ s/\&lsquo\;(.*?)XXXrsquoXXX/\&lsquo\;<article-title>$1<\/article-title>\&rsquo\;/;
		}
	}
	$content =~ s/\[\[rsquo\]\]/XXXrsquoXXX/sgi;
	$content =~ s/([\.\,])(&ldquo;<article-title>)/$1 $2/;
	$content =~ s/(<\/article-title>[\,\.]\&rdquo\;)([A-Za-z])/$1 $2/;
	
	if($content !~ m/<Journal_Title>/) {
		if($content =~ m/<\/article-title>[\,\.]?(\&rdquo\;|\&rsquo\;)[\,\.]? ((?:The )?Journal [A-Za-z ]+)([\.\,])/i) {
			$content =~ s/(<\/article-title>[\,\.]?(?:\&rdquo\;|\&rsquo\;)[\,\.]?) ((?:The )?Journal [A-Za-z ]+)([\.\,])/$1 <Journal_Title>$2<\/Journal_Title>$3/i;
		} elsif($content =~ m/<\/article-title>[\,\.]?(\&rdquo\;|\&rsquo\;)[\,\.]? ((?:The )?Proceedings [A-Za-z 0-9]+)([\.\,])/i) {
			$content =~ s/(<\/article-title>[\,\.]?(?:\&rdquo\;|\&rsquo\;)[\,\.]?) ((?:The )?Proceedings [A-Za-z 0-9]+)([\.\,])/$1 <Journal_Title>$2<\/Journal_Title>$3/i;
		} elsif($content =~ m/<\/article-title>[\,\.]?(\&rdquo\;|\&rsquo\;)[\,\.]? ([A-Za-z\- ]+)([\.\,]? <volume>)/i) {
			$content =~ s/(<\/article-title>[\,\.]?(?:\&rdquo\;|\&rsquo\;)[\,\.]?) ([A-Za-z\- ]+)([\.\,]? <volume>)/$1 <Journal_Title>$2<\/Journal_Title>$3/i;
...
sub InsertDoi {
	my $text = shift;
  if ($text =~ m/(<\/Journal_Title>|<\/lpage>)\, 10\.\d{4}\/([\-\S\.\(\)]+)/){
  	
    $text =~ s/(<\/Journal_Title>|<\/lpage>)\, (10\.\d{4}\/[\-\S\.\(\)]+)/$1\, <Doi>$2<\/Doi>/;
...
sub year_aftr_page_conversion {
	my $content = shift;
	if($content !~ m/<Year>/) {
		if($content =~ m/(\,| )((18|19|20)[0-9][0-9][a-z]?)[\.\,] (pp?.? )?<fpage>/) {
			$content =~ s/(\,| )((18|19|20)[0-9][0-9][a-z]?)([\.\,] (pp?.? )?<fpage>)/$1<Year>$2<\/Year>$4/;
		}
	}
	return $content;
}
...
sub Publoc_aftr_Pubname_conversion {
	my $content = shift;
  foreach my $tagName ('PublisherLocation','fpage','Year') {
...
sub restrict_book_conversion {
  ...
  if($content !~ m/<chapter-title>/ && $content !~ m/<article-title>/ && $content !~ m/<Journal_Title>/) {
  ...
  if($content !~ m/<PublisherLocation>/ && $content !~ m/<PublisherName>/) {
    ...
    if($content !~ m/<PublisherLocation>/ && $content !~ m/<PublisherName>/) {
        ...
        if(exists $Countrycode{$country}) {
          ...
          $content =~ s/([\,\;\.] | and |\()($country)( ?\: ?)((?:Dept|Inst)\. [A-Za-z\-\&\; ]+|[A-Za-z\-\&\; ]+)(\)|[\,\.])/$1<PublisherLocation>$2<\/PublisherLocation>$3<PublisherName>$4<\/PublisherName>$5/i;
          $flag = 1;
        }
        if($content !~ m/<PublisherLocation>/
					&& $content !~ m/<PublisherName>/) {
...
### Need to create as subroutine
  ...
  #print $content,"\n";
  $content =~ s/<PublisherName><\/article-title>/<\/article-title><PublisherName>/g;
  ...
  if($content !~ m/<PublisherLocation>/) {
 		if($content =~ m/([\,\;\.] )(Washington\, D\.?C\.?|London\, UK|New Delhi|New York|San Francisco|Los Angeles|U\.K\.)\./) {
 		...
 		} elsif($content =~ m/([\,\;\.] )(USA|U\.K\.), pp\. <fpage>/) {
 			$content =~ s/([\,\;\.] )(USA|U\.K\.), pp\. <fpage>/$1<PublisherLocation>$2<\/PublisherLocation>, pp. <fpage>/;
 		} elsif($content =~ m/([\,\;\.] )(U\.K\.)$/) {
 			$content =~ s/([\,\;\.] )(U\.K\.)$/$1<PublisherLocation>$2<\/PublisherLocation>/;
 		} elsif($content =~ m/([\,\;\.] )([A-Za-z]+)\, pp\. <fpage>/) {
 			my $country = $2;
 			my $testf = 0;
 			...
 			if($testf == 1) {
 				$content =~ s/([\,\;\.] )($country)\, pp\. <fpage>/$1<PublisherLocation>$2<\/PublisherLocation>, pp\. <fpage>/;
 			}
...
sub publisherloc_tagging {
  ...
  $content = &Reftagging::year_aftr_Publoc_conversion($content, "fpage");
  return $content;
}
...
sub missing_Journaltitle {
	my $content = shift;
	if($content !~ m/<Journal_Title>/) {
		$content =~ s/(<\/article-title>[\,\.](?:\&rdquo\;)? )([A-Z][^><]+)((?:Vol\.)? <volume>)/$1<Journal_Title>$2<\/Journal_Title>$3/i;
	}
	if($content !~ m/<Journal_Title>/) {
		$content =~ s/(<\/article-title>(?:\&rdquo\;)[\,\.] )([A-Z][^><]+)((?:Vol\.)? <volume>)/$1<Journal_Title>$2<\/Journal_Title>$3/i;
	}
	#print $content,"\n";
	if($content !~ m/<Journal_Title>/) {
		$content =~ s/(<\/article-title>(?:\,?\&rdquo\;)[\,\.]* )(<I>[A-Z][^><]+<\/I>[\.\, ]*)((?:Vol\.)? <volume>)/$1<Journal_Title>$2<\/Journal_Title>$3/i;
	}
	if($content !~ m/<Journal_Title>/) {
		$content =~ s/(<I>[A-Z][^><]+<\/I>[\.\, ]*)((?:Vol\.)? <volume>)/<Journal_Title>$1<\/Journal_Title>$2/i;
	}
	return $content;
}
...
#print $content,"\n\n";
    ...
    elsif($content =~ m/\(([0-9]{4}[a-z]?)\)[\.\,]* &\#8220;(.*?)&\#8221;[\,\.]? (.*?)<strong>[0-9]+<\/strong>\([0-9]+\)/i)
    {
      $content =~ s/<p>(.*?)<\/p>/<p><Journal>$1<\/Journal><\/p>/i;
    }
    elsif($content =~ m/\(([0-9]{4}[a-z]?)\)[\.\,]* ?&\#8220;(.*?)&\#8221;[\,\.]? (.*?)<strong>[0-9]+<\/strong>[\,\:] [0-9]+(\&\#173\;|\&\#8211\;|\-)[0-9]+/i)
    {
      $content =~ s/<p>(.*?)<\/p>/<p><Journal>$1<\/Journal><\/p>/i;
    }
    elsif($content =~ m/\(([0-9]{4}[a-z]?)\)[\.\,]* &\#8220;(.*?)&\#8221;[\,\.]? (.*?), [0-9]+\([0-9]+\)/i)
    {
      $content =~ s/<p>(.*?)<\/p>/<p><Journal>$1<\/Journal><\/p>/i;
    }
    elsif($content =~ m/\(([0-9]{4}[a-z]?)\)[\.\,]* &\#8220;(.*?)&\#8221;[\,\.]? (.*?), [0-9]+\, [0-9]+(\&\#173\;|\&\#8211\;|\-)[0-9]+/i)
    {
      $content =~ s/<p>(.*?)<\/p>/<p><Journal>$1<\/Journal><\/p>/i;
    }
    elsif($content =~ m/\(([0-9]{4}[a-z]?)\)[\.\,]* ?&\#8220;(.*?)&\#8221;[\,\.]? (.*?) [0-9]+[\,\:] [0-9]+(\&\#173\;|\&\#8211\;|\-)[0-9]+/i)
...
sub journal_conversion {
  ...
  if ($journal =~ m/ \(([0-9]{4}[a-z]?)\)/) {
    ...
    $journal =~ s/ ((?:19|20)[0-9]{2}[a-z]?)([\.\,]*) (.*?)(\.|\,) <em>(.*?)<\/em>/ <Year>$1<\/Year>$2 <article-title>$3<\/article-title>$4 <Journal_Title><em>$5<\/em><\/Journal_Title>/;
    ...
    if ($journal =~ m/&ldquo;(.*?)&rdquo; ([A-Za-z0-9\(\)\&\;\:\#\-\s]+)[\.\,] ([0-9]+)\, ($month)?\s*((?:18|19|20)[0-9]{2}[a-z]?)\, (pp?\. )?([0-9]+)\s*(-|&ndash;|&mdash;|&\#8211;)\s*([0-9]+)(\.?)$/) {
      $journal =~ s/&ldquo;(.*?)&rdquo; ([A-Za-z0-9\(\)\&\;\:\#\-\s]+)([\.\,]) ([0-9]+)\, ($month)?(\s*)?((?:18|19|20)[0-9]{2}[a-z]?)\, (pp?\. )?([0-9]+)\s*(-|&ndash;|&mdash;|&\#8211;)\s*([0-9]+)(\.?)$/&ldquo;<article-title>$1<\/article-title>&rdquo; <Journal_Title>$2<\/Journal_Title>$3 <volume>$4<\/volume>\, <Year>$5$6$7<\/Year>\, $8<fpage>$9<\/fpage>&ndash;<lpage>$11<\/lpage>$12/;
    } elsif ($journal =~ m/ ((18|19|20)[0-9]{2}[a-z]?)(\.?)$/) {
  ...
  if ($journal =~ m/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; <em>/) {
    $journal =~ s/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; <em>/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; <em>/;
  } elsif ($journal =~ m/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; ASCE\, <em>/) {
    $journal =~ s/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; ASCE\, <em>/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; ASCE\, <em>/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) &ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.]*) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, (Vo?l?\. )?([0-9]+)\,/) {
    $journal =~ s/<\/Year>\)([\,\.]*) &ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.]*) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, ([0-9]+)\,/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo;$3 <Journal_Title>$4<\/Journal_Title>\, <volume>$5<\/volume>\,/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) &ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.]*) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, (([D|d]oi|DOI):)/) {
    $journal =~ s/<\/Year>\)([\,\.]*) &ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.]*) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, (([D|d]oi|DOI):)/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo;$3 <Journal_Title>$4<\/Journal_Title>\, $5/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*\s*)&ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+)\,(\s*[0-9]+)/) {
    $journal =~ s/<\/Year>\)([\,\.]*\s*)&ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+)\,(\s*[0-9]+)/<\/Year>\)$1&ldquo;<article-title>$2<\/article-title>&rdquo;$3$4\,$5/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*\s*)&ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+) ([0-9]+)/) {
    $journal =~ s/<\/Year>\)([\,\.]*\s*)&ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+) ([0-9]+)/<\/Year>\)$1&ldquo;<article-title>$2<\/article-title>&rdquo;$3$4 $5/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) &ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.\s]*)([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, <volume>/) {
    $journal =~ s/<\/Year>\)([\,\.]*) &ldquo;([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)&rdquo;([\,\.\s]*)([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, <volume>/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo;$3 $4\, <volume>/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, (Vo?l?\. )?<volume>/) {
    $journal =~ s/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, (Vo?l?\. )?<volume>/<\/Year>\)$1 <article-title>$2<\/article-title>$3 $4\, $5<volume>/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)[\,\.] ([0-9\/]+|[0-9]+\([0-9\/]+\))\,/) {
    $journal =~ s/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]) ([0-9\/]+|[0-9]+\([0-9\/]+\))\,/<\/Year>\)$1 <article-title>$2<\/article-title>$3 $4$5 $6\,/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)[\,\.] ([0-9\/]+|[0-9]+\([0-9\/]+\)) ([0-9]+)/) {
    $journal =~ s/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]) ([0-9\/]+|[0-9]+\([0-9\/]+\)) ([0-9]+)/<\/Year>\)$1 <article-title>$2<\/article-title>$3 $4$5 $6 $7/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)[\,\.]* ([0-9\/]+|[0-9]+\([0-9]+\))\: ([0-9]+)/) {
    $journal =~ s/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]?) ([0-9\/]+|[0-9]+\([0-9]+\))\: ([0-9]+)/<\/Year>\)$1 <article-title>$2<\/article-title>$3 $4$5 $6\: $7/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)[\,\.]* ([0-9\/]+|[0-9]+\([0-9]+\))\, ([0-9]+)/) {
    $journal =~ s/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\#\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]?) ([0-9\/]+|[0-9]+\([0-9]+\))\, ([0-9]+)/<\/Year>\)$1 <article-title>$2<\/article-title>$3 $4$5 $6\, $7/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)[\,\.] (([D|d]oi|DOI):)/) {
    $journal =~ s/<\/Year>\)([\,\.]*) ([A-Za-z0-9\(\)\?\:\&\;\-\.\s]+)([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]) ((?:Doi|doi|DOI):)/<\/Year>\)$1 <article-title>$2<\/article-title>$3 $4$5 $6/;
  }
  print "$journal\n" if ($debug);
  
  # journal title conversion
  if ($journal =~ m/<\/article-title>&rdquo; <em>(.*?)<\/em>/) {
    $journal =~ s/<\/article-title>&rdquo; <em>(.*?)<\/em>/<\/article-title>&rdquo; <Journal_Title><em>$1<\/em><\/Journal_Title>/;
  } elsif ($journal =~ m/<\/article-title>&rdquo; ASCE\, <em>(.*?)<\/em>/) {
    $journal =~ s/<\/article-title>&rdquo; ASCE\, <em>(.*?)<\/em>/  &rdquo; ASCE\, <Journal_Title><em>$1<\/em><\/Journal_Title>/;
  } elsif ($journal =~ m/<\/article-title>\&rdquo\;[\,\.]*\s*([A-Za-z\(\)\#\:\&\;\-\.\s]+)\,\s*([0-9]+)/) {
    $journal =~ s/<\/article-title>\&rdquo\;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+)\,(\s*[0-9]+)/<\/article-title>\&rdquo\;$1<Journal_Title>$2<\/Journal_Title>\,$3/;
  } elsif ($journal =~ m/<\/article-title>\&rdquo\;[\,\.]*\s*([A-Za-z\(\)\#\:\&\;\-\.\s]+) ([0-9]+)\, ([0-9]+)/) {
    $journal =~ s/<\/article-title>\&rdquo\;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+) ([0-9]+)\, ([0-9]+)/<\/article-title>\&rdquo\;$1<Journal_Title>$2<\/Journal_Title> $3\, $4/;
  } elsif ($journal =~ m/<\/article-title>\&rdquo\;[\,\.]*\s*([A-Za-z\(\)\#\:\&\;\-\.\s]+) ([0-9]+)[\,\:] ([A-Z]?[0-9]+)\,/) {
    $journal =~ s/<\/article-title>\&rdquo\;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+) ([0-9]+)([\,\:]) ([A-Z]?[0-9]+)\,/<\/article-title>\&rdquo\;$1<Journal_Title>$2<\/Journal_Title> $3$4 $5\,/;
  } elsif ($journal =~ m/<\/article-title>\&rdquo\;[\,\.]*\s*([A-Za-z\(\)\#\:\&\;\-\.\s]+) ([0-9]+\([0-9]+\))[\,\:]/) {
    $journal =~ s/<\/article-title>\&rdquo\;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+) ([0-9]+\([0-9]+\))([\,\:])/<\/article-title>\&rdquo\;$1<Journal_Title>$2<\/Journal_Title> $3$4/;
  } elsif ($journal =~ m/<\/article-title>\&rdquo\;[\,\.]*\s*([A-Za-z\(\)\#\:\&\;\-\.\s]+)[\,\.]* ([0-9]+)[\,\:]/) {
    $journal =~ s/<\/article-title>\&rdquo\;([\,\.]*\s*)([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]*) ([0-9]+)([\,\:])/<\/article-title>\&rdquo\;$1<Journal_Title>$2<\/Journal_Title>$3 $4$5/;
  } elsif ($journal =~ m/<\/Year>\)([\,\.]*) &ldquo;([A-Za-z0-9\(\)\#\:\&\;\-\.\s]+)&rdquo;([\,\.]*) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\,/ && $journal=~ m/in press/i) {
    $journal =~ s/<\/Year>\)([\,\.]*) &ldquo;([A-Za-z0-9\(\)\#\:\&\;\-\.\s]+)&rdquo;([\,\.]*) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\,/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo;$3 <Journal_Title>$4<\/Journal_Title>\,/;
  } elsif ($journal =~ m/<\/article-title>\&rdquo\;[\,\.\s]*([A-Za-z\(\)\#\:\&\;\-\.\s]+)[\,\.]? <volume>/) {
    $journal =~ s/<\/article-title>\&rdquo\;([\,\.\s]*)([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]?) <volume>/<\/article-title>\&rdquo\;$1<Journal_Title>$2<\/Journal_Title>$3 <volume>/;
  } elsif ($journal =~ m/<\/article-title>\&rdquo\;[\,\.\s]*([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.\s]*)(doi:\s*[\S]+\.?)$/) {
    $journal =~ s/<\/article-title>\&rdquo\;([\,\.\s]*)([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]*) (doi:\s*[\S]+\.?)/<\/article-title>\&rdquo\;$1<Journal_Title>$2<\/Journal_Title>$3 $4/;
  } elsif ($journal =~ m/<\/article-title>[\,\.] ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, (Vo?l?\. )?<volume>/) {
    $journal =~ s/<\/article-title>([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, (Vo?l?\. )?<volume>/<\/article-title>$1 <Journal_Title>$2<\/Journal_Title>\, $3<volume>/;
  } elsif ($journal =~ m/<\/article-title>[\,\.] ([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]) ((?:Doi|doi|DOI):\s*[\S]+\.?)$/) {
    $journal =~ s/<\/article-title>([\,\.]) ([A-Za-z\(\)\#\:\&\;\-\.\s]+)([\,\.]) ((?:Doi|doi|DOI):\s*[\S]+\.?)$/<\/article-title>$1 <Journal_Title>$2<\/Journal_Title>$3 $4/;
  } elsif ($journal =~ m/<\/article-title>[\,\.] ([A-Za-z\(\)\:\&\;\-\.\s]+)\, ([0-9\/]+|[0-9\/]+\([0-9\/]+\))\,/) {
    $journal =~ s/<\/article-title>([\,\.]) ([A-Za-z\(\)\:\&\;\-\.\s]+)\, ([0-9\/]+|[0-9\/]+\([0-9\/]+\))\,/<\/article-title>$1 <Journal_Title>$2<\/Journal_Title>\, $3,/;
  } elsif ($journal =~ m/<\/article-title>[\,\.] ([A-Za-z\(\)\:\&\;\-\.\s]+)[\,\.]? ([0-9\/]+|[0-9\/]+\([0-9\/]+\))\: ([0-9]+)/) {
    $journal =~ s/<\/article-title>([\,\.]) ([A-Za-z\(\)\:\&\;\-\.\s]+)([\,\.]?) ([0-9\/]+|[0-9\/]+\([0-9\/]+\))\: ([0-9]+)/<\/article-title>$1 <Journal_Title>$2<\/Journal_Title>$3 $4\: $5/;
  }
  print "$journal\n" if ($debug);
  ...
  if ($journal !~ m/<article-title>/ && $journal !~ m/<Journal_Title>/) {
    $journal =~ s/&ldquo;(.+)(,|\.\?)&rdquo;/&ldquo;<article-title>$1<\/article-title>${2}&rdquo;/g;
	$journal =~ s/&\#34;(.+)(,|\.\?)&\#34;/&\#34;<article-title>$1<\/article-title>${2}&\#34;/g;
  ...
  if ($journal =~ m/<\/Journal_Title>([\,\.]?) <strong>([0-9]+)<\/strong> ?\(((?:[A-Z])?[0-9]+|[0-9]+(?:&ndash;|\-|&minus;|&mdash;)[0-9]+)\)([\,\:])/) {
    $journal =~ s/<\/Journal_Title>([\,\.]?) <strong>([0-9]+)<\/strong> ?\(((?:[A-Z])?[0-9]+)\)([\,\:])/<\/Journal_Title>$1 <volume><strong>$2<\/strong><\/volume>\(<issue>$3<\/issue>\)$4/;
    $journal =~ s/<\/Journal_Title>([\,\.]?) <strong>([0-9]+)<\/strong> ?\(([0-9]+)(&ndash;|\-|&minus;|&mdash;)([0-9]+)\)([\,\:])/<\/Journal_Title>$1 <volume><strong>$2<\/strong><\/volume>\(<issue>$3&ndash;$5<\/issue>\)$6/;
  } elsif ($journal =~ m/<\/Journal_Title>([\,\.]?) <strong>([0-9]+)<\/strong>([\,\: ])/) {
    $journal =~ s/<\/Journal_Title>([\,\.]?) <strong>([0-9]+)<\/strong>([\,\: \(])/<\/Journal_Title>$1 <volume><strong>$2<\/strong><\/volume>$3/;
  } elsif ($journal =~ m/<\/Journal_Title>([\,\.]?) <strong>([0-9]+)<\/strong>([\,\:\(])/) {
    $journal =~ s/<\/Journal_Title>([\,\.]?) <strong>([0-9]+)<\/strong>([\,\: \(])/<\/Journal_Title>$1 <volume><strong>$2<\/strong><\/volume>$3/;
  }

  if ($journal =~ m/<\/Journal_Title>([\,\.]?\s*)([0-9\/]+)\(((?:[A-Z])?[0-9\/]+|[0-9]+(?:&ndash;|\-|&minus;|&mdash;)[0-9]+)\)([\,\:])/) {
    $journal =~ s/<\/Journal_Title>([\,\.]?\s*)([0-9\/]+)\(((?:[A-Z])?[0-9\/]+)\)([\,\:])/<\/Journal_Title>$1<volume>$2<\/volume>\(<issue>$3<\/issue>\)$4/;
    $journal =~ s/<\/Journal_Title>([\,\.]?\s*)([0-9\/]+)\(([0-9]+)(&ndash;|\-|&minus;|&mdash;)([0-9]+)\)([\,\:])/<\/Journal_Title>$1<volume>$2<\/volume>\(<issue>$3&ndash;$5<\/issue>\)$6/;
  } elsif ($journal =~ m/<\/Journal_Title>([\,\.]) ([0-9\/]+|[0-9]+(?:&ndash;|\-|&minus;|&mdash;)[0-9]+)([\,\:])/) {
    $journal =~ s/<\/Journal_Title>([\,\.]) ([0-9\/]+|[0-9]+(?:&ndash;|\-|&minus;|&mdash;)[0-9]+)([\,\:])/<\/Journal_Title>$1 <volume>$2<\/volume>$3/;
  } elsif ($journal =~ m/<\/Journal_Title>([\,\.]) Vol ([0-9]+)([\,\:])/) {
    $journal =~ s/<\/Journal_Title>([\,\.]) Vol ([0-9]+)([\,\:])/<\/Journal_Title>$1 Vol <volume>$2<\/volume>$3/;
  } elsif ($journal =~ m/<\/Journal_Title>[\.\,]? ([0-9]+)\, ([A-Z][0-9]+)\, doi:/) {
    $journal =~ s/<\/Journal_Title>([\.\,]?) ([0-9]+)\, ([A-Z]?[0-9]+)\, doi:/<\/Journal_Title>$1 <volume>$2<\/volume>\, <fpage>$3<\/fpage>\, doi:/;
  } elsif ($journal =~ m/<\/Journal_Title>[\.\,]? ([0-9]+)\, doi:/) {
    $journal =~ s/<\/Journal_Title>([\.\,]?) ([0-9]+)\, doi:/<\/Journal_Title>$1 <volume>$2<\/volume>\, doi:/;
  } elsif ($journal =~ m/<\/Journal_Title> ([0-9]+)\, ([A-Z]?[0-9]+)(?:&ndash;|\-|&minus;|&mdash;)([A-Z]?[0-9]+)\, (doi:)/i) {
    $journal =~ s/<\/Journal_Title> ([0-9]+)\, ([A-Z]?[0-9]+)(?:&ndash;|\-|&minus;|&mdash;)([A-Z]?[0-9]+)\, (doi:)/<\/Journal_Title> <volume>$1<\/volume>\, <fpage>$2<\/fpage>&ndash;<lpage>$3<\/lpage>\, $4/i;
  }
#print $journal,"\n";


  if ($journal =~ m/ ([0-9]+)(\s*)\([0-9]+[a-zA-Z]?\)(\,? )([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)(\.?)$/) {
    $journal =~ s/ ([0-9]+)(\s*)\(([0-9]+[a-zA-Z]?)\)(\,? )([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)(\.?)$/ <volume>$1<\/volume>$2\(<issue>$3<\/issue>\)$4<fpage>$5<\/fpage>&ndash;<lpage>$7<\/lpage>$8/;
  } elsif ($journal =~ m/ ([0-9]+)\:([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)(\.?)$/) {
    $journal =~ s/ ([0-9]+)\:([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)(\.?)$/ <volume>$1<\/volume>\:<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
  } elsif ($journal =~ m/ ([0-9]+)\:([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)[, ]+\(/) {
    $journal =~ s/ ([0-9]+)\:([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)([, ]+\()/ <volume>$1<\/volume>\:<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
  } elsif ($journal =~ m/(<\/volume>|<\/issue>\)?)\,? ([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)[, ]+\(/) {
    $journal =~ s/(<\/volume>|<\/issue>\)?)\,? ([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)([, ]+\()/${1}, <fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
  } elsif ($journal =~ m/ ([A-Z]?[0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([A-Z]?[0-9]+|XXX)[, ]+\(/) {
    $journal =~ s/ ([A-Z]?[0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([A-Z]?[0-9]+|XXX)([, ]+\()/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
  }
#  elsif ($journal =~ m/ (e?[A-Z]?[0-9]+|XXX)[, ]+\(/) {
#    $journal =~ s/ (e?[A-Z]?[0-9]+|XXX)([, ]+\()/ <fpage>$1<\/fpage>$2/;
#  }


  if ($journal =~ m/ ([0-9]+)(\:\s*)([0-9]+)(\.?)$/) {
    $journal =~ s/ ([0-9]+)(\:\s*)([0-9]+)(\.?)$/ <volume>$1<\/volume>$2<fpage>$3<\/fpage>/;
  }
    
  if ($journal =~ m/<\/volume>(: ?)([0-9]+)\.$/) {
    $journal =~ s/<\/volume>(: ?)([0-9]+)\.$/<\/volume>$1<fpage>$2<\/fpage>\./;
  }
  if ($journal =~ m/<\/volume>\, ([A-Z]?[0-9]+)\.$/) {
    $journal =~ s/<\/volume>\, ([A-Z]?[0-9]+)\.$/<\/volume>\, <fpage>$1<\/fpage>\./;
  }

  if ($journal =~ m/<\/issue>\): ([0-9]+)\.$/) {
    $journal =~ s/<\/issue>\): ([0-9]+)\.$/<\/issue>\): <fpage>$1<\/fpage>\./;
  }
  
  if ($journal =~ m/<\/issue>\)\, ([0-9]+) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+)\,/) {
    $journal =~ s/<\/issue>\)\, ([0-9]+) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+)\,/<\/issue>\)\, <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>\,/;
  }

  if ($journal =~ m/(\:| )([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)(\.?)$/) {
    $journal =~ s/(\:| )([0-9]+|XXX) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+|XXX)(\.?)$/$1<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
  }
  
  if ($journal =~ m/<\/Journal_Title>\, [0-9]+(?:[A-Z])?\([0-9]+((\-|&minus;|&ndash;|&mdash;)[0-9]+)?\)\.$/) {
    $journal =~ s/<\/Journal_Title>\, ([0-9]+(?:[A-Z])?)\(([0-9]+((\-|&minus;|&ndash;|&mdash;)[0-9]+)?)\)\.$/<\/Journal_Title>\, <volume>$1<\/volume>\(<issue>$2<\/issue>\)\./;
  }

  if ($journal =~ m/<\/volume>\, ([A-Z]?[0-9]+[A-Z]?[0-9]+)\, doi:/) {
    $journal =~ s/<\/volume>\, ([A-Z]?[0-9]+[A-Z]?[0-9]+)\, doi:/<\/volume>\, <fpage>$1<\/fpage>\, doi:/;
  }

  if ($journal =~ m/<\/volume>\, ([0-9]+) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+) /) {
    $journal =~ s/<\/volume>\, ([0-9]+) ?(-|&ndash;|&minus;|&mdash;) ?([0-9]+) /<\/volume>, <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage> /;
  }
  ...
  if ($journal !~ m/<article-title>/) {
    if ($journal =~ m/<\/Year>\)\.? \&ldquo\;(.*?)\&rdquo\;/) {
      $journal =~ s/(<\/Year>\)\.? )\&ldquo\;(.*?)\&rdquo\;/$1\&ldquo\;<article-title>$2<\/article-title>\&rdquo\;/i;
    }
  }
  ...
  if ($journal =~ m/<article-title>(.*?)<\/article-title>/) {
    my $articletitle = $1;
    if ($articletitle =~ m/\.$/) {
      my ($before, $match, $after) = ($`, $&, $');
      if ($before !~ m/\./) {
        $journal =~ s/\.<\/article-title>/<\/article-title>\./;
      }
    }
  }
  ...
  if ($journal !~ m/<volume>/) {
    if ($journal =~ m/ ([0-9]+)(\:\s*)<fpage>/) {
      $journal =~ s/ ([0-9]+)(\:\s*)<fpage>/ <volume>$1<\/volume>$2<fpage>/;
    } elsif ($journal =~ m/ ([0-9]+)\(<issue>/) {
      $journal =~ s/ ([0-9]+)\(<issue>/ <volume>$1<\/volume>\(<issue>/;
    }
  }
  #print $journal,"\n\n";

  if ($journal !~ m/<Journal_Title>/) {
    if ($journal =~ m/<\/article-title>\.?\&rdquo\;\.?\,? ([A-Za-z\(\)\#\:\&\;\-\.\s\,]+)\,\.?? ([V|v]ol. )?<volume>/) {
      $journal =~ s/<\/article-title>\&rdquo\;([\.\,]) ([A-Za-z\(\)\:\#\&\;\-\.\,\s]+)([\,\.]?) ([V|v]ol. )?<volume>/<\/article-title>\&rdquo\;$1 <Journal_Title>$2<\/Journal_Title>$3 $4<volume>/;
      $journal =~ s/ ([V|v]ol\.)<\/Journal_Title>/<\/Journal_Title> $1/;
      $journal =~ s/<\/article-title>\.\&rdquo\; ([A-Za-z\(\)\:\#\&\;\-\.\,\s]+)\, ([V|v]ol. )?<volume>/<\/article-title>\.\&rdquo\; <Journal_Title>$1<\/Journal_Title>\, $2<volume>/;
      $journal =~ s/<\/article-title>\.\&rdquo\; ([A-Za-z\(\)\:\#\&\;\-\.\,\s]+) ([V|v]ol. )?<volume>/<\/article-title>\.\&rdquo\; <Journal_Title>$1<\/Journal_Title> $2<volume>/;
      $journal =~ s/<\/article-title>\&rdquo\; ([A-Za-z\(\)\:\#\&\;\-\.\,\s]+)\, ([V|v]ol. )?<volume>/<\/article-title>\&rdquo\; <Journal_Title>$1<\/Journal_Title>\, $2<volume>/;
    }
    if ($journal =~ m/<\/article-title>\. ([A-Za-z\(\)\#\:\&\;\-\.\s]+)[\,\.]? <volume>/) {
      $journal =~ s/<\/article-title>\. ([A-Za-z\(\)\:\#\&\;\-\.\s]+)([\,\.]) <volume>/<\/article-title>\. <Journal_Title>$1<\/Journal_Title>$2 <volume>/;
      $journal =~ s/<\/article-title>\. ([A-Za-z\(\)\:\#\&\;\-\.\s]+)([\,\.]?) <volume>/<\/article-title>\. <Journal_Title>$1<\/Journal_Title>$2 <volume>/;
    }
    if ($journal =~ m/<\/article-title>\&rdquo\;\.? ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\,? doi:/) {
      $journal =~ s/<\/article-title>\&rdquo\;\. ([A-Za-z\(\)\#\:\&\;\-\.\s]+)\, doi:/<\/article-title>\&rdquo\;\. <Journal_Title>$1<\/Journal_Title>\, doi:/;
      $journal =~ s/<\/article-title>\&rdquo\;\. ([A-Za-z\(\)\#\:\&\;\-\.\s]+) doi:/<\/article-title>\&rdquo\;\. <Journal_Title>$1<\/Journal_Title> doi:/;
    }
  }
...
sub book_conversion {
  ...
  if ($book =~ m/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo;( in | |\, |\, chapter [0-9]+ in )<em>(.*?)<\/em>\,/) {
    $book =~ s/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo;( in | |\, |\, chapter [0-9]+ in )<em>(.*?)<\/em>\,/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo;$3<Journal_Title><em>$4<\/em><\/Journal_Title>\,/;
  }

  if ($book =~ m/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo; Chapter ([0-9]+)\, <em>(.*?)<\/em>\,/) {
    $book =~ s/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo; Chapter ([0-9]+)\, <em>(.*?)<\/em>\,/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; Chapter $3\, <Journal_Title><em>$4<\/em><\/Journal_Title>\,/;
  }
  ...
  if ($book =~ m/<\/Journal_Title>\, Vol\. ([0-9]+)\,/) {
    $book =~ s/<\/Journal_Title>\, Vol\. ([0-9]+)\,/<\/Journal_Title>\, Vol\. <volume>$1<\/volume>\,/;
  }

  if ($book =~ m/ ed\.\, Vol\. ([0-9]+)\,/) {
    $book =~ s/ ed\.\, Vol\. ([0-9]+)\,/ ed\.\, Vol\. <volume>$1<\/volume>\,/;
  }
...
#  }
  ...
  if ($book =~ m/<\/volume>\, ([A-z\s\&\;\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $book =~ s/<\/volume>\, ([A-z\s\&\;\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/<\/volume>\, <PublisherName>$1<\/PublisherName>\, <PublisherLocation>$2<\/PublisherLocation>\, <fpage>$3<\/fpage>&ndash;<lpage>$5<\/lpage>$6/;
  }

  if ($book =~ m/<\/volume>\, ([A-z\s\&\;\-]+)\, ([A-z\s\-]+)([\.]?)$/) {
    $book =~ s/<\/volume>\, ([A-z\s\&\;\-]+)\, ([A-z\s\-]+)([\.]?)$/<\/volume>\, <PublisherName>$1<\/PublisherName>\, <PublisherLocation>$2<\/PublisherLocation>$3/;
  }

  if ($book =~ m/ (\(Eds\.?\)|eds\.|ed\.)\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([A-Z]+|[A-z\.]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $book =~ s/ (\(Eds\.?\)|eds\.|ed\.)\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([A-Z]+|[A-z\.]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/ $1\, <PublisherName>$2<\/PublisherName>\, <PublisherLocation>$3\, $4<\/PublisherLocation>\, <fpage>$5<\/fpage>&ndash;<lpage>$7<\/lpage>$8/;
  }

  if ($book =~ m/ (\(Eds\.?\)|eds\.)\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $book =~ s/ (\(Eds\.?\)|eds\.)\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/ $1\, <PublisherName>$2<\/PublisherName>\, <PublisherLocation>$3<\/PublisherLocation>\, <fpage>$4<\/fpage>&ndash;<lpage>$6<\/lpage>$7/;
  }
  ...
  if ($book =~ m/<\/Journal_Title>\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $book =~ s/<\/Journal_Title>\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/<\/Journal_Title>\, <PublisherName>$1<\/PublisherName>\, <PublisherLocation>$2<\/PublisherLocation>\, <fpage>$3<\/fpage>&ndash;<lpage>$5<\/lpage>$6/;
  }

  if ($book =~ m/ ([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)\.$/) {
    $book =~ s/ ([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)\.$/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>\./;
  }

  if ($book =~ m/ ([0-9\.]+) pp([\.]?)$/) {
    $book =~ s/ ([0-9\.]+) pp([\.]?)$/ <fpage>$1<\/fpage> pp$2/;
  }

  if ($book =~ m/ ([0-9]+)\.$/) {
    $book =~ s/ ([0-9\.]+)\.$/ <fpage>$1<\/fpage>\./;
  }
...
#  }
  ...
  if ($book !~ m/<volume>/) {
    if ($book =~ m/\, Vol\. ([0-9]+)\,/) {
      $book =~ s/\, Vol\. ([0-9]+)\,/\, Vol\. <volume>$1<\/volume>\,/;
    }
  }
...
#Inserting Chapter title..    
  $book =~ s/<article-title>(.*?)<\/article-title>/<chapter-title>$1<\/chapter-title>/;
  return $book;
}
...
#      } elsif ($conference =~ m/<\/Year>.*?<article-title>/) {
#        $conference =~ s/^(.*?) <Year>/<Authorgroup>$1<\/Authorgroup> <Year>/;
#      }
#    }
  } else {
    $conference =~ s/&ldquo;(.+),&rdquo;/&ldquo;<article-title>$1<\/article-title>,&rdquo;/;
...
#    }
  ...
  if ($conference =~ m/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; <em>/) {
    $conference =~ s/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; <em>/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; <em>/;
  }
  if ($conference =~ m/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo; (Proceedings?)/) {
    $conference =~ s/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo; (Proceedings?)/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; $3/;
  }
  if ($conference =~ m/<\/article-title>&rdquo; <em>(.*?)<\/em>/) {
    $conference =~ s/<\/article-title>&rdquo; <em>(.*?)<\/em>/<\/article-title>&rdquo; <ConferenceName><em>$1<\/em><\/ConferenceName>/;
  }
  if ($conference =~ m/<\/article-title>&rdquo; (Proceedings? of) ([A-Za-z0-9\&\;\#\?\s]+)\,/) {
    $conference =~ s/<\/article-title>&rdquo; (Proceedings? of) ([A-Za-z0-9\&\;\#\?\s]+)\,/<\/article-title>&rdquo; <ConferenceName>$1 $2<\/ConferenceName>\,/;
  }
  ...
  if ($conference =~ m/<\/ConferenceName>\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $conference =~ s/<\/ConferenceName>\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/<\/ConferenceName>\, <ConferenceLocation>$1\, $2<\/ConferenceLocation>\, <fpage>$3<\/fpage>&ndash;<lpage>$5<\/lpage>$6/;
  } elsif ($conference =~ m/ ([A-Z]?[0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([A-Z]?[0-9]+|XXX)[, ]+\(/) {
    $conference =~ s/ ([A-Z]?[0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([A-Z]?[0-9]+|XXX)([, ]+\()/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
  }
  ...
  if ($conference =~ m/<\/ConferenceName>\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([A-Z]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $conference =~ s/<\/ConferenceName>\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([A-Z]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/<\/ConferenceName>\, <ConferenceLocation>$1\, $2\, $3<\/ConferenceLocation>\, <fpage>$4<\/fpage>&ndash;<lpage>$6<\/lpage>$7/;
  }
  ...
  if ($conference =~ m/<\/ConferenceName>([\,\.]?) <strong>([0-9]+)<\/strong> ?\(((?:[A-Z])?[0-9]+|[0-9]+(?:&ndash;|\-|&minus;|&mdash;)[0-9]+)\)([\,\:])/) {
    $conference =~ s/<\/ConferenceName>([\,\.]?) <strong>([0-9]+)<\/strong> ?\(((?:[A-Z])?[0-9]+)\)([\,\:])/<\/ConferenceName>$1 <volume><strong>$2<\/strong><\/volume>\(<issue>$3<\/issue>\)$4/;
    $conference =~ s/<\/ConferenceName>([\,\.]?) <strong>([0-9]+)<\/strong> ?\(([0-9]+)(&ndash;|\-|&minus;|&mdash;)([0-9]+)\)([\,\:])/<\/ConferenceName>$1 <volume><strong>$2<\/strong><\/volume>\(<issue>$3&ndash;$5<\/issue>\)$6/;
  } elsif ($conference =~ m/<\/ConferenceName>([\,\.]?) <strong>([0-9]+)<\/strong>([\,\: ])/) {
    $conference =~ s/<\/ConferenceName>([\,\.]?) <strong>([0-9]+)<\/strong>([\,\: \(])/<\/ConferenceName>$1 <volume><strong>$2<\/strong><\/volume>$3/;
  } elsif ($conference =~ m/<\/ConferenceName>([\,\.]?) <strong>([0-9]+)<\/strong>([\,\:\(])/) {
    $conference =~ s/<\/ConferenceName>([\,\.]?) <strong>([0-9]+)<\/strong>([\,\: \(])/<\/ConferenceName>$1 <volume><strong>$2<\/strong><\/volume>$3/;
  }
        
  if ($conference =~ m/<\/ConferenceName>([\,\.]?) ([0-9]+)\(((?:[A-Z])?[0-9]+|[0-9]+(?:&ndash;|\-|&minus;|&mdash;)[0-9]+)\)([\,\:])/) {
    $conference =~ s/<\/ConferenceName>([\,\.]?) ([0-9]+)\(((?:[A-Z])?[0-9]+)\)([\,\:])/<\/ConferenceName>$1 <volume>$2<\/volume>\(<issue>$3<\/issue>\)$4/;
    $conference =~ s/<\/ConferenceName>([\,\.]?) ([0-9]+)\(([0-9]+)(&ndash;|\-|&minus;|&mdash;)([0-9]+)\)([\,\:])/<\/ConferenceName>$1 <volume>$2<\/volume>\(<issue>$3&ndash;$5<\/issue>\)$6/;
  } elsif ($conference =~ m/<\/ConferenceName>([\,\.]) ([0-9]+)([\,\:])/) {
    $conference =~ s/<\/ConferenceName>([\,\.]) ([0-9]+)([\,\:])/<\/ConferenceName>$1 <volume>$2<\/volume>$3/;
  } elsif ($conference =~ m/<\/ConferenceName>([\,\.]) Vol ([0-9]+)([\,\:])/) {
    $conference =~ s/<\/ConferenceName>([\,\.]) Vol ([0-9]+)([\,\:])/<\/ConferenceName>$1 Vol <volume>$2<\/volume>$3/;
  }

  if ($conference =~ m/ ([0-9]+)\:([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)(\.?)$/) {
    $conference =~ s/ ([0-9]+)\:([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)(\.?)$/ <volume>$1<\/volume>\:<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
  } elsif ($conference =~ m/ ([0-9]+)\:([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)[, ]+\(/) {
    $conference =~ s/ ([0-9]+)\:([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)([, ]+\()/ <volume>$1<\/volume>\:<fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
  } elsif ($conference =~ m/(<\/volume>|<\/issue>\)?)\,? ([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)[, ]+\(/) {
    $conference =~ s/(<\/volume>|<\/issue>\)?)\,? ([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)([, ]+\()/${1}, <fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage>$5/;
  } elsif ($conference =~ m/ ([A-Z]?[0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([A-Z]?[0-9]+|XXX)[, ]+\(/) {
    $conference =~ s/ ([A-Z]?[0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([A-Z]?[0-9]+|XXX)([, ]+\()/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
  }
  
  if ($conference =~ m/ ([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)\.$/) {
    $conference =~ s/ ([0-9]+|XXX) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+|XXX)\.$/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>\./;
  }
  ...
  if ($conference !~ m/<ConferenceName>/) {
    if ($conference =~ m/<\/article-title>&rdquo;\.? ([A-Za-z0-9\(\)\&\;\#\.\/\?\s]+)\,/) {
      my $confNameText = $1;
      if ($confNameText =~ m/(International Technical Meeting|Proceeding of|International Symposium of|Congress|International Symposium on)/i) {
        $conference =~ s/<\/article-title>&rdquo;\. ([A-Za-z0-9\(\)\&\;\#\.\/\?\s]+)\,/<\/article-title>&rdquo;\. <ConferenceName>$1<\/ConferenceName>\,/i;
      }
    }
  }
  return $conference;
...
#    } elsif ($thesis =~ m/<\/Year>.*?<article-title>/) {
#      $thesis =~ s/^(.*?) <Year>/<Authorgroup>$1<\/Authorgroup> <Year>/ if($thesis !~ m/<Authorgroup>/);
#    }
  }
  else
  {
    $thesis =~ s/&ldquo;(.+),&rdquo;/&ldquo;<article-title>$1<\/article-title>,&rdquo;/;
...
#    }
  ...
  if ($thesis =~ m/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo;( in | |\, |\, chapter [0-9]+ in )<em>(.*?)<\/em>\,/) {
    $thesis =~ s/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo;( in | |\, |\, chapter [0-9]+ in )<em>(.*?)<\/em>\,/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo;$3<Journal_Title><em>$4<\/em><\/Journal_Title>\,/;
  }

  if ($thesis =~ m/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo; Chapter ([0-9]+)\, <em>(.*?)<\/em>\,/) {
    $thesis =~ s/<\/Year>\)([\.]?) &ldquo;(.*?)&rdquo; Chapter ([0-9]+)\, <em>(.*?)<\/em>\,/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; Chapter $3\, <Journal_Title><em>$4<\/em><\/Journal_Title>\,/;
  }
  ...
  if ($thesis =~ m/<\/Journal_Title>\, Vol\. ([0-9]+)\,/) {
    $thesis =~ s/<\/Journal_Title>\, Vol\. ([0-9]+)\,/<\/Journal_Title>\, Vol\. <volume>$1<\/volume>\,/;
  }

  if ($thesis =~ m/ ed\.\, Vol\. ([0-9]+)\,/) {
    $thesis =~ s/ ed\.\, Vol\. ([0-9]+)\,/ ed\.\, Vol\. <volume>$1<\/volume>\,/;
  }
  ...
  if ($thesis =~ m/<\/volume>\, ([A-z\s\&\;\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $thesis =~ s/<\/volume>\, ([A-z\s\&\;\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/<\/volume>\, <PublisherName>$1<\/PublisherName>\, <PublisherLocation>$2<\/PublisherLocation>\, <fpage>$3<\/fpage>&ndash;<lpage>$5<\/lpage>$6/;
  }

  if ($thesis =~ m/<\/volume>\, ([A-z\s\&\;\-]+)\, ([A-z\s\-]+)([\.]?)$/) {
    $thesis =~ s/<\/volume>\, ([A-z\s\&\;\-]+)\, ([A-z\s\-]+)([\.]?)$/<\/volume>\, <PublisherName>$1<\/PublisherName>\, <PublisherLocation>$2<\/PublisherLocation>$3/;
  }
   
  if ($thesis =~ m/ (\(Eds\.?\)|eds\.|ed\.)\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([A-Z]+|[A-z\.]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $thesis =~ s/ (\(Eds\.?\)|eds\.|ed\.)\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([A-Z]+|[A-z\.]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/ $1\, <PublisherName>$2<\/PublisherName>\, <PublisherLocation>$3\, $4<\/PublisherLocation>\, <fpage>$5<\/fpage>&ndash;<lpage>$7<\/lpage>$8/;
  }

  if ($thesis =~ m/ (\(Eds\.?\)|eds\.)\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $thesis =~ s/ (\(Eds\.?\)|eds\.)\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/ $1\, <PublisherName>$2<\/PublisherName>\, <PublisherLocation>$3<\/PublisherLocation>\, <fpage>$4<\/fpage>&ndash;<lpage>$6<\/lpage>$7/;
  }
  ...
  if ($thesis =~ m/<\/Journal_Title>\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
    $thesis =~ s/<\/Journal_Title>\, ([A-z\s\-]+)\, ([A-z\s\-]+)\, ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/<\/Journal_Title>\, <PublisherName>$1<\/PublisherName>\, <PublisherLocation>$2<\/PublisherLocation>\, <fpage>$3<\/fpage>&ndash;<lpage>$5<\/lpage>$6/;
  }
  ...
  if ($thesis =~ m/ ([0-9\.]+) pp([\.]?)$/) {
    $thesis =~ s/ ([0-9\.]+) pp([\.]?)$/ <fpage>$1<\/fpage> pp$2/;
  }
  ...
  if ($thesis !~ m/<article-title>/) {
    if ($thesis =~ m/<\/Year>\)\.? \&ldquo\;(.*?)\&rdquo\;/) {
      $thesis =~ s/(<\/Year>\)\.? )\&ldquo\;(.*?)\&rdquo\;/$1\&ldquo\;<article-title>$2<\/article-title>\&rdquo\;/i;
    }
  }
...
sub report_conversion {
  ...
  if ($report !~ m/<\/Year>\)[.,]? &ldquo;/ && $report =~ m/<\/Year>\)\.?$/) {
    $report =~ s/&ldquo;(.+)(,|\.\?)&rdquo;/&ldquo;<article-title>$1<\/article-title>${2}&rdquo;/g;
  ...
  if ($report =~ m/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; <em>/) {
    $report =~ s/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; <em>/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; <em>/;
  }

  if ($report =~ m/<\/article-title>&rdquo; <em>(.*?)<\/em>/) {
    $report =~ s/<\/article-title>&rdquo; <em>(.*?)<\/em>/<\/article-title>&rdquo; <Journal_Title><em>$1<\/em><\/Journal_Title>/;
  }
...
sub other_conversion {
  my $references = shift;
    ...
    if ($references !~ m/<\/Year>\)[.,]? &ldquo;/ && $references =~ m/<\/Year>\)\.?$/) {
      $references =~ s/&ldquo;(.+)(,|\.\?)&rdquo;/&ldquo;<article-title>$1<\/article-title>${2}&rdquo;/g;
    ...
    if ($references =~ m/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; <em>/) {
      $references =~ s/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; <em>/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; <em>/;
    } elsif ($references =~ m/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; ASCE\, <em>/) {
      $references =~ s/<\/Year>\)(\.?) &ldquo;(.*?)&rdquo; ASCE\, <em>/<\/Year>\)$1 &ldquo;<article-title>$2<\/article-title>&rdquo; ASCE\, <em>/;
    } elsif ($references =~ m/<\/Year>[\.\,]? &ldquo;(.*?)&rdquo;/ && $references !~ m/<article-title>/) {
      $references =~ s/(<\/Year>[\.\,\)]? )&ldquo;(.*?)&rdquo;/$1&ldquo;<article-title>$2<\/article-title>&rdquo;/;
    }

    if ($references =~ m/&rdquo; <em>(.*?)<\/em>/) {
      $references =~ s/&rdquo; <em>(.*?)<\/em>/&rdquo; <Journal_Title><em>$1<\/em><\/Journal_Title>/;
    } elsif ($references =~ m/<\/article-title>&rdquo; ASCE\, <em>(.*?)<\/em>/) {
      $references =~ s/<\/article-title>&rdquo; ASCE\, <em>(.*?)<\/em>/<\/article-title>&rdquo; ASCE\, <Journal_Title><em>$1<\/em><\/Journal_Title>/;
    }
    ...
    if ($references =~ m/p ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
      $references =~ s/p ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)/p <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
    }

    if ($references =~ m/ ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/) {
      $references =~ s/ ([0-9\.]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9\.]+)([\.]?)$/ <fpage>$1<\/fpage>&ndash;<lpage>$3<\/lpage>$4/;
    }

    if ($references =~ m/ ([0-9\.]+) pp([\.]?)$/) {
      $references =~ s/ ([0-9\.]+) pp([\.]?)$/ <fpage>$1<\/fpage> pp$2/;
    }

    
    if ($references =~ m/<strong>[0-9]+<\/strong>\, no\. <issue>/) {
      $references =~ s/<strong>([0-9]+)<\/strong>\, no\. <issue>/<volume><strong>$1<\/strong><\/volume>\, no\. <issue>/;
    }
    if ($references =~ m/<strong>[0-9]+<\/strong>\, <fpage>/) {
      $references =~ s/<strong>([0-9]+)<\/strong>\, <fpage>/<volume><strong>$1<\/strong><\/volume>\, <fpage>/;
    }
#    if ($references =~ m/ [Nn]o\. [0-9]+\, <fpage>/) {
#      $references =~ s/ ([Nn]o\.) ([0-9]+)\, <fpage>/ $1 <issue>$2<\/issue>\, <fpage>/;
#    }

    if ($references =~ m/\b[Vv]ol\. [0-9]+\, <fpage>/) {
      $references =~ s/\b([Vv]ol\.) ([0-9]+)\, <fpage>/$1 <volume>$2<\/volume>\, <fpage>/;
    }

    if ($references =~ m/<em>([^\,]+)<\/em>\,? <volume>/) {
      $references =~ s/<em>([^\,]+)<\/em>(\,)? <volume>/<Journal_Title><em>$1<\/em><\/Journal_Title>$2 <volume>/;
    }

    if ($references =~ m/<em>([^\,]+)<\/em>\. <volume>/) {
      $references =~ s/<em>([^\,]+)<\/em>\. <volume>/<Journal_Title><em>$1\.<\/em><\/Journal_Title> <volume>/;
    }
    ...
    if ($references !~ m/<article-title>/) {
      if ($references =~ m/<\/Year>\)\.? \&ldquo\;(.*?)\&rdquo\;/) {
        $references =~ s/(<\/Year>\)\.? )\&ldquo\;(.*?)\&rdquo\;/$1\&ldquo\;<article-title>$2<\/article-title>\&rdquo\;/i;
      }
    }
    ...
    if ($references !~ m/<volume>/) {
      if ($references =~ m/\, Vol\. ([0-9]+)\,/) {
        $references =~ s/\, Vol\. ([0-9]+)\,/\, Vol\. <volume>$1<\/volume>\,/;
      }
    }
    
    if ($references =~ m/ [0-9]+(?:[A-Z])?\([0-9]+((\-|&minus;|&ndash;|&mdash;)[0-9]+)?\)\, <fpage>/) {
      $references =~ s/ ([0-9]+(?:[A-Z])?)\(([0-9]+((\-|&minus;|&ndash;|&mdash;)[0-9]+)?)\)\, <fpage>/ <volume>$1<\/volume>(<issue>$2<\/issue>)\, <fpage>/;
    }
    
    if ($references =~ m/ \([0-9]+\)\, [0-9]+ ?(-|&ndash;|&mdash;|&minus;) ?[0-9]+ \(in/) {
      $references =~ s/ (\([0-9]+\))\, ([0-9]+) ?(-|&ndash;|&mdash;|&minus;) ?([0-9]+) \(in/ $1\, <fpage>$2<\/fpage>&ndash;<lpage>$4<\/lpage> \(in/;
    }
...
sub Cleanup {
  ...
  $line =~ s{<p>(.*?)<\/p>}
  {
	...
	$content =~ s/<p><bold>(\[?[0-9\.]+\]? ?)<\/bold>/$1<p>/gi;
...
sub PatternMatching {
	...
	if($Refstyles =~ m/\[REFS\](.*?\n)\s*\[[A-Z]+\]/si)	{
		...
		$dbtext =~ s/<article-title>/$patattitle/gi;
		$dbtext =~ s/<Chap_Title>/$patchaptitle/gi;
		$dbtext =~ s/<Journal_Title>/$patjrnltitle/gi;
		$dbtext =~ s/<Extra_Info>/$patextrainfo/gi;
		$dbtext =~ s/<volume>/$patvolume/gi;
		$dbtext =~ s/<issue>/$patissue/gi;
		$dbtext =~ s/<Suppl_No>/$patsupplement/gi;
		$dbtext =~ s/<fpage>/$patfpage/gi;
		$dbtext =~ s/<lpage>/$patlpage/gi;
	...
	@edbk = ($dbtext =~ m/edbk=(.*?)\n/sgi);
		...
		$text =~ s/<article-title>/<article-title><doller><\/article-title>/gi;
		...
		$text =~ s/<volume>/<volume><doller><\/volume>/gi;
		$text =~ s/<issue>/<issue><doller><\/issue>/gi;
		$text =~ s/<Suppl_No>/<Suppl_No><doller><\/Suppl_No>/gi;
		$text =~ s/<fpage>/<fpage><doller><\/fpage>/gi;
		$text =~ s/<lpage>/<lpage><doller><\/lpage>/gi;
